...
def newton_grad(x,y,w):
    h = np.zeros((x.shape[1],x.shape[1]))
    for xi,yi in zip(x,y):
        exp = np.exp(-yi*np.dot(xi,w.reshape(-1,1)))
        h += exp/(1+exp)**2*np.dot(xi.reshape(-1,1),xi.reshape(1,-1))
    h += wd
    invh = np.linalg.inv(h)
    return np.dot(grad(x,y,w),invh.transpose())
